...
export default function CancelledOrdersPage() {
  ...
  // Fetch orders data
  useEffect(() => {
    console.log('Cancelled Orders useEffect triggered, user:', user);
    if (user && user.hotel_id) {
      console.log('Fetching cancelled orders for hotel_id:', user.hotel_id);
  ...
  const fetchCancelledOrders = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/hotels/backoffice/ordercancelled/${user.hotel_id}`, {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          console.warn('Orders API endpoint not found or no data available');
          // ไม่มีข้อมูล ให้แสดง array ว่าง
          setOrders([]);
          return;
        }
      ...
      // จัดการ timeout error และ network error
      if (err.name === 'AbortError') {
        console.warn('API request timed out, no data available');
        setOrders([]);
      } else {
        console.warn('Network error or no data available:', err.message);
        setOrders([]);
      }
    } finally {
      setLoading(false);
    ...
    try {
      ...
      let filteredOrders = originalOrders.filter(order => {
        ...
        if (searchCategory && matches) {
          ...
          if (!category.includes(searchCategoryLower)) {
            matches = false;
          }
        }
      ...
      if (filteredOrders.length === 0) {
        showErrorAlert('ไม่พบคำสั่งซื้อที่ยกเลิกที่ตรงกับเงื่อนไขการค้นหา');
      } else {
        showSuccessAlert(`พบคำสั่งซื้อที่ยกเลิก ${filteredOrders.length} รายการ`);
      }
    } catch (err) {
      console.error('Error searching cancelled orders:', err);
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Manage and view cancelled bookings</p>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                <Link 
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {orders.filter(order => order.payment === 3).length}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">Pending Refund</p>
                  <p className="text-2xl font-bold text-gray-900">
                    {orders.filter(order => order.payment === 2).length}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    ¥{Array.isArray(orders) ? orders.reduce((total, order) => total + (parseFloat(order.sum_price) || 0), 0).toLocaleString() : '0'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm mb-6">
            <div className="px-4 py-3 border-b border-gray-200">
              <h3 className="text-sm font-medium text-gray-900">Search Cancelled Orders</h3>
            </div>
            <div className="p-4">
              <div className="flex flex-wrap gap-4 justify-end">
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    className="w-40 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 text-sm text-right text-gray-900"
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-medium text-gray-900">Cancelled Orders List</h2>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {orders.length === 0 ? (
                    <tr>
                      <td colSpan={13} className="text-center py-8 text-gray-400">No orders were cancelled.</td>
                    </tr>
                  ) : (
                    orders.map((order) => (
                      <tr key={order.number_order} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-800">
                          <a
                            href={`/cancelled-orders/${order.number_order}`}
                            target="_blank"
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full transform transition-all duration-300 scale-100">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Cancelled Order Details</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      <span className="text-sm font-medium text-gray-600">Cancelled Date:</span>
