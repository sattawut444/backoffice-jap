...
export default function OrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/hotels/backoffice/order/${user.hotel_id}`, {
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center space-x-4">
                <Link 
                  ...
                  className="flex items-center space-x-2 text-green-700 hover:text-green-900 px-4 py-3 rounded-lg font-medium transition-all duration-200"
                >
                  <div className="flex items-center justify-center w-8 h-8 bg-green-100 hover:bg-green-200 rounded-full transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-3xl font-bold text-gray-900">
                  {Array.isArray(orders) && orders.length > 0
                    ? orders.filter(order => order.status_confirm === 1).length
                    : 0}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-all duration-200">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-3xl font-bold text-gray-900">
                  {Array.isArray(orders) && orders.length > 0
                    ? orders.filter(order => order.status_confirm === 0).length
                    : 0}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-all duration-200">
              <div className="flex flex-col items-center text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-red-500 to-red-600 rounded-full flex items-center justify-center shadow-lg mb-4">
                  <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <p className="text-3xl font-bold text-gray-900">
                  {Array.isArray(orders) && orders.length > 0
                    ? orders.filter(order => order.status_confirm === 2).length
                    : 0}
                </p>
              </div>
            </div>
                    ...
                    orders.map((order, index) => (
                      ...
                      <tr key={order.number_order} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                          <div>
                            <div className="text-sm font-semibold text-gray-800">{order.room_name}</div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex px-2 py-1 text-xs font-bold rounded-full border ${getStatusConfirmedColor(order.status_confirm)}`}>
                            {order.message || getStatusText(order.status_confirm)}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          <div className="flex items-center space-x-2">
                            ...
                            {(order.status_confirm === 2 || order.status_confirm === 0) && (
                              <button 
                                ...
                                onClick={() => handleUpdateStatus(order.number_order || order.number_order, 1)}
                              >
                                Confirm
                              </button>
                            )}
                            {/* Show Cancel button for Pending or Confirmed orders */}
                            {(order.status_confirm === 2 || order.status_confirm === 1 || order.status_confirm === 0) && (
                              <button 
                                ...
                                onClick={() => handleUpdateStatus(order.number_order || order.number_order, 2)}
                              >
                                Cancel
                              </button>
                            )}
