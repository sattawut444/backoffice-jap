...
export default function OrderDetailPage() {
  ...
  useEffect(() => {
    async function fetchOrder() {
      ...
      try {
        ...
        setError("Order information not found or an error occurred.");
      } finally {
        setLoading(false);
      }
    }
  ...
  if (!rooms || rooms.length === 0) return <div className="p-8 text-center text-gray-500">Order information not found</div>;
  ...
  let torel_sum_price = 0;
  for(const item of rooms){
    torel_sum_price += parseInt(item.price);
  }
  return (
    <div className="max-w-3xl mx-auto p-6 md:p-10 bg-gradient-to-br from-white via-gray-50 to-gray-100 rounded-3xl shadow-2xl mt-10 border border-gray-200">
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-10">
        ...
        <div className="bg-white/80 rounded-2xl p-6 shadow border border-gray-100">
          ...
          <div className="space-y-2 text-gray-600 text-base">
            <div>
              <span className="font-medium">Sum Price:</span>
              <span > ¥{torel_sum_price}</span>
            </div>
            <div>
              <span className="font-medium">Payment:</span>
              <span className={`ml-2 px-3 py-1 rounded-full text-xs font-bold shadow-sm border transition-all ${getPaymentStatusColor(order.payment)}`}>{getpaymentStatusText(order.payment)}</span>
            </div>
            <div>
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {rooms.map((room, idx) => (
                <tr key={idx} className="hover:bg-indigo-50/60 transition">
                  ...
                  <td className="px-6 py-3 whitespace-nowrap text-sm text-indigo-700 font-semibold">{room.price ? `¥${parseFloat(room.price).toLocaleString()}` : '-'}</td>
